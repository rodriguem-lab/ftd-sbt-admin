    ...
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <section className="rounded-xl bg-white shadow-sm border border-gray-200 p-6">
        ...
        <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
          <div className="rounded-xl border border-gray-200 p-4 bg-white">
            <div className="text-gray-600">Contract (Sepolia)</div>
            <div className="mt-1 font-mono break-all text-gray-900">
              {CONTRACT_ADDRESS}
            </div>
          </div>

          <div className="rounded-xl border border-gray-200 p-4 bg-white">
            <div className="text-gray-600">Réseau connecté</div>
            <div className="mt-1">
              {isConnected ? (
                isSepolia ? (
                  <span className="text-emerald-600 font-medium">Sepolia ✅</span>
                ) : (
                ...
                <span className="text-gray-700">Non connecté</span>
              )}
            </div>

            {!isSepolia && isConnected && (
              <button
                className="mt-3 px-3 py-2 rounded-lg bg-gray-900 text-white text-sm disabled:opacity-60"
          ...
          <div className="rounded-xl border border-gray-200 p-4 bg-white">
            <div className="text-gray-600">Owner (owner())</div>
            <div className="mt-1 font-mono break-all text-gray-900">
              {owner ? String(owner) : "…"}
            </div>
          </div>
          ...
          <div className="rounded-xl border border-gray-200 p-4 bg-white">
            <div className="text-gray-600">Wallet connecté = owner ?</div>
            <div className="mt-1">
              {!isConnected ? (
      ...
      <section className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="rounded-xl bg-white shadow-sm border border-gray-200 p-6">
          ...
          <input
            className="mt-4 w-full rounded-lg bg-white border border-gray-300 px-3 py-2 text-sm font-mono text-gray-900 placeholder:text-gray-400"
            placeholder="0x..."
            value={mintTo}
          ...
          <button
            className="mt-3 w-full px-3 py-2 rounded-lg bg-gray-900 text-white text-sm disabled:opacity-50"
            onClick={onMint}
          ...
          <div className="mt-2 text-xs text-gray-600">
            {canAdmin ? (
              ...
              <span className="text-red-600">Non autorisé / mauvais réseau ❌</span>
            )}
          </div>
        </div>
        ...
        <div className="rounded-xl bg-white shadow-sm border border-gray-200 p-6">
          ...
          <div className="mt-3 flex items-center justify-between gap-3">
            <label className="text-sm text-gray-700">
              ...
              <input
                type="number"
                className="ml-2 w-20 rounded-lg bg-white border border-gray-300 px-2 py-1 text-sm text-gray-900"
          ...
          <textarea
            className="mt-3 w-full h-28 rounded-lg bg-white border border-gray-300 px-3 py-2 text-xs font-mono text-gray-900 placeholder:text-gray-400"
            placeholder="0xabc...\n0xdef...\n..."
            value={batchText}
      ...
      <section className="rounded-xl bg-white shadow-sm border border-gray-200 p-6">
        <div className="flex items-center justify-between gap-3 flex-wrap">
          ...
          <div className="flex gap-2">
            <button
              ...
              onClick={() =>
                downloadText(`batch-template-${Date.now()}.txt`, "0x...\n0x...\n")
              }
            >
              Template
            </button>
        ...
        <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-3 text-sm">
          <div className="rounded-xl border border-gray-200 p-4 bg-white">
            <div className="text-gray-600">Tx state</div>
          ...
          <div className="rounded-xl border border-gray-200 p-4 bg-white">
            <div className="text-gray-600">Dernière tx hash</div>
            <div className="mt-1 font-mono break-all text-gray-900">
              {txHash ?? "—"}
            </div>
          </div>
          ...
          <div className="rounded-xl border border-gray-200 p-4 bg-white">
            <div className="text-gray-600">Minted (session)</div>
        ...
        <div className="mt-4 space-y-2">
          {logs.length === 0 ? (
            <div className="text-sm text-gray-500">Aucun log pour le moment.</div>
          ) : (
            logs.map((l, idx) => (
              <div
                key={idx}
              ...
              >
                <div className="flex flex-wrap gap-x-3 gap-y-1 items-center text-xs text-gray-600">
                ...
                {l.hash && (
                  <div className="mt-1 text-xs font-mono break-all text-gray-900">
                    {l.hash}
                  </div>
                )}
                {l.note && (
                  <div className="mt-1 text-xs text-gray-700">{l.note}</div>
                )}
              </div>
            ))
          )}
